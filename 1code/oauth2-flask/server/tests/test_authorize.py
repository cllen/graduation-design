...
class AuthorizeTestCase(unittest.TestCase):
    ...
    def test_login(self):
        ...
        response = self.client.post('/oauth2/authorization/authorize', data={
            'account': TestingConfig.testing_student_account,
            'password': TestingConfig.testing_student_password,
        ...
        self.assertEqual(json_response.get('entry').get('username'), TestingConfig.testing_student_name)
